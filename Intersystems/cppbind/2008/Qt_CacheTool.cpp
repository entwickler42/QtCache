...
void Qt_CacheTool::setErrorLog(const d_string& val)
{
   ...
   __db->set_property(get_ref(), __prop_name, 1, 0, 1, __args_mgr);
}
...
d_string Qt_CacheTool::getErrorLog() const
{
   ...
   Critical_section cs(__db->get_cs_handle());
   __db->get_property(get_ref(), __prop_name, 1, 0, __args_mgr);
   return __res;
}

void Qt_CacheTool::setLastStatus(const d_status& val)
{
   GEN_DECLARE_ARGS_AND_DB();
   ...
   __args_mgr.set_next(&val);
   Critical_section cs(__db->get_cs_handle());
   __db->set_property(get_ref(), __prop_name, -2, 0, 2, __args_mgr);
}
...
d_status Qt_CacheTool::getLastStatus() const
{
   ...
   __args_mgr.set_next_as_res(&__res);
   Critical_section cs(__db->get_cs_handle());
   __db->get_property(get_ref(), __prop_name, 2, 0, __args_mgr);
   return __res;
}
...
d_status Qt_CacheTool::CompileList(const d_string& uci, const d_string& objectNames, const d_string& qspec)
{
   GEN_DECLARE_ARGS_AND_DB();
