...
private:
   ...
   Qt_CacheTool(Database* db, int oref, const wchar_t* cl_name)
   { init(db, oref, cl_name); }

public:
   virtual ~Qt_CacheTool() {}

   static d_ref<Qt_CacheTool> openref(t_istream& in, Database* db)
   {
   ...
   static d_ref<Qt_CacheTool> openref(Database* db, int oref, const_name_t cl_name)
   {
      if (oref == 0)
         return d_ref<Qt_CacheTool>();

      Obj_t *obj; int *num_refs;
      if (db->get_proxies_info()->get_proxy_info(oref, typeid(Qt_CacheTool), &obj, &num_refs))
         return d_ref<Qt_CacheTool>(dynamic_cast<Qt_CacheTool*>(obj), num_refs);
      else
         return d_ref<Qt_CacheTool>(new Qt_CacheTool(db, oref, cl_name));
   }

   static d_ref<Qt_CacheTool> create_new(Database* db, const_str_t init_val = 0, Db_err* err = 0)
   {
   ...
   virtual void setErrorLog(const d_string& val);
   virtual d_string getErrorLog() const;

   virtual void setLastStatus(const d_status& val);
